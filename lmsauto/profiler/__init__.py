...
class SystemProfiler:
    ...
    def _get_gpu_info(self) -> Dict[str, str]:
        ...
        if _torch_available:
            try:
                # Explicitly ignore type checking for torch if it's optionally imported
                # This avoids Pylance errors when torch is not installed.
                if torch.cuda.is_available(): # type: ignore
                    gpu_available = "Yes (CUDA)"
                    device_count: int = torch.cuda.device_count() # type: ignore
                    details_list: list[str] = [] # Explicit type hint
                    total_vram: float = 0.0 # Explicit type hint and initialization
                    for i in range(device_count):
                         device_name: str = torch.cuda.get_device_name(i) # type: ignore
                         try:
                             vram: float = torch.cuda.get_device_properties(i).total_memory / BYTES_TO_GB # type: ignore
                             total_vram += vram
