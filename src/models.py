...
class LMStudioAPI:
    ...
    def apply_settings(self, model_name: str, settings: Dict[str, Any]) -> bool:
...
class ModelProfileManager:
    ...
    def save_profile(self, model_name: str, settings: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"Saving profile for model {model_name} to {profile_path}")
    ...
    def load_profile(self, model_name: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dictionary containing model settings if profile exists, None otherwise.
        """
        profile_path = f"{self.profiles_dir}/{model_name}.json"
        try:
            return load_json(profile_path)
        except FileNotFoundError:
            self.logger.warning(f"No profile found for model {model_name}")
            return None
        except Exception as e:
            self.logger.error(f"Error loading profile for model {model_name}: {e}")
            return None